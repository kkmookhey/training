...
TOOL_PERMISSIONS = {
    "Tier 1": ["get_ticket_details"],
    "Tier 2": ["get_ticket_details", "run_vulnerability_scan"]
}

# --- Secure Agent Creation Logic ---
def create_secure_agent_executor(role: str):
    llm = ChatOpenAI(model_name="gpt-4o", temperature=0)

    # Only provide the tools the agent is allowed to use for its role
    allowed_tool_names = TOOL_PERMISSIONS.get(role, [])
    all_tools = {"get_ticket_details": get_ticket_details, "run_vulnerability_scan": run_vulnerability_scan}
    tools = [all_tools[name] for name in allowed_tool_names]

    if not tools:
        return None # Handle case where role has no tools

    # The prompt can now be simpler as rules are enforced by code
...
if 'agent_role' not in st.session_state:
    st.session_state.agent_role = "Tier 1"
    ...
    with st.chat_message("assistant"):
        with st.spinner("Agent is thinking..."):
            try:
                ...
                agent_executor = create_secure_agent_executor(st.session_state.agent_role)
